...
export function isPgcPlaybackPage(href = window.location.href) {
  try {
    const url = new URL(href)
    return url.hostname === 'www.bilibili.com' && /^\/bangumi\/play\/(?:ss|ep)\d+\/?$/.test(url.pathname)
  }
  catch {
    return false
  }
...
export function isNativeVideoComponentReady(node: HTMLElement): boolean | undefined {
  ...
  if (!parseVideoPageIdentity(href) && !isPgcPlaybackPage(href))
    return true
...
export function releaseNativeVideoComponent(node: HTMLElement) {
  if (!node.hasAttribute('data-bewly-react-bridge'))
    return
  node.dispatchEvent(new CustomEvent(VIDEO_COMPONENT_REQUEST, {
    bubbles: true,
    detail: JSON.stringify({ channelId: getPageBridgeChannelId(), requestId: ++requestId, href: location.href, release: true }),
  }))
}
