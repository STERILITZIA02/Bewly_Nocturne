...
- 2026-09-09：私信会话容器保留左右两条语义细线；滚动、到达历史起点、分页或返回最新消息均不改变展开形态。首次打开才展开容器，切换用户仅淡出消息记录并逐条显示新历史，外框和输入栏保持同一实例；再次点击当前用户关闭详情，不通过 history.back 恢复前一个用户。换号立即清理旧视图，离场记录不可继续交互。输入栏底部与左侧列表底部对齐，外壳复用播放页底部容器的 modal 圆角、边框、材质和边缘阴影，内部输入区使用匹配嵌套关系的普通实色表面。
