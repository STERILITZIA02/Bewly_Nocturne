...
- 2026-09-09：私信会话容器保留左右两条语义细线；滚动、到达历史起点、分页或返回最新消息均不改变展开形态。切换会话先淡出旧视图，再展开新视图并逐条显示历史；换号立即清理旧视图，离场 DOM 不可继续交互。输入栏外壳复用播放页底部容器的 modal 圆角、边框、材质和边缘阴影，内部输入区使用匹配嵌套关系的普通实色表面。
