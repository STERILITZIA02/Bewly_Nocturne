...
| 原视频页模块 | 当前状态 | Bewly 播放页路径 / 限制 |
| --- | --- | --- |
| 播放器画面与控制栏 | 支持 | 整个原播放器节点搬入 player slot，播放、进度、清晰度、倍速、字幕、音量、设置、画中画和全屏控制继续使用原监听器。 |
| 弹幕发送栏与在线观看人数 | 支持 | 原发送栏搬入播放器下方 danmaku dock。 |
| 视频标题 | 部分支持 | 播放页复制标题文字；原 `video-info-title` 模块及其附加徽标/链接没有整体搬入。 |
| 播放量、弹幕数、发布时间 | 未接入 | 原 `video-info-meta/detail` 仍留在被播放页覆盖的页面中。 |
...
| 三连动画与操作状态 | 支持 | 原节点与状态保留，并有播放页内动画几何校正。 |
| 稍后再看 | 支持 | 复用现有独立按钮和 `topBarStore` 权威成员状态。 |
| 工具栏右侧其他入口 | 未完整接入 | 当前播放页样式只保留稍后再看；笔记等其他 right-side action 会被隐藏。 |
...
| 播放器外 AI 章节/AI 总结助手 | 未接入 | `_VideoAssistant / video-ai-assistant / _SeekSections` 没有播放页 content adapter 分区。 |
...
| 投币、收藏、分享、举报等 body 弹窗 | 条件支持 | 入口保留，弹窗继续由 Bilibili 挂到播放页根外。主播放页根必须保持普通 overlay 层级，加载遮罩单独置顶，才能让这些弹窗显示在播放页之上。 |
| 广告、活动横幅和商业推广 | 有意排除 | 不属于目标接入范围。 |

## 实时页面观察

当前浏览器中的普通 UGC 视频显示：播放器、弹幕发送栏、UP/联合创作信息、主互动工具栏、简介、标签、选集和推荐已进入 Bewly 播放页；原视频 meta 与版权声明仍在播放页根外。浏览器当前加载的是重新编译前的扩展实例，评论 Tab 仍显示 loading，因此只能作为旧问题证据，不能替代最终构建重载后的验收。

## 最终结论

Bewly 播放页**不能宣称原视频播放页除广告/活动外的所有模块均可访问**。它目前覆盖普通观看所需的核心播放、UP、互动、简介、标签、弹幕、评论、选集和推荐主链；但 meta/版权、AI、笔记、音乐、荣誉/争议信息和部分工具栏右侧功能仍缺少正式接入。
