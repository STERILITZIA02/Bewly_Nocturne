...
- Dock 默认位于底部；`Notifications` 是可配置的独立 Bewly 混合消息页：`reply / at / love` 使用 Vue Native Feed，`whisper` 是使用原生会话列表与普通用户/官方助手只读历史的 hybrid workspace，`system / settings` 使用完整的原版消息 iframe fallback。未关注、fold、拦截和非普通会话以及全部写操作继续由原版页面承载，不得一次性移植或重写完整私信客户端。System 已完成当前发布 bundle 的静态协议调查，但仍缺少登录态成功首屏响应、历史分页成功响应、`update_cursor` 成功响应与 `sys_msg` 归零时序；在这些真实脱敏 fixture 齐备前必须保持 original iframe，禁止根据旧 `Stage_2_fix` 或归档文档猜测 System 数据结构。
