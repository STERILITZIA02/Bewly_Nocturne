...
- Native 私信当前只读；文本与图片发送 transport、optimistic/reconcile 与上传 transaction 必须完整保留在明确标记的 experimental 目录，但生产 API、页面 shell 和组件运行时不得导入或触发。2026-08-14 的真实文本发送尝试返回 HTTP `412`，被 transport 分类为 `risk-control` 并生成结构化 code `-412`（未观察到合法 JSON code）；服务端历史未出现该消息，因此协议门禁仍为 blocked。现有合成 `code=0` fixture 不得视为服务端协议验证；未经用户明确批准以及新的真实 `code=0`、服务端 history confirmation 和图片链路成功证据，不得重新开放 Composer，也不得删除 experimental 代码或负向 fixture。
